...
contract Token is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (
            ...
            if (!takeFee) {
                ...
                amount = _takeBuyFees(to ,from, amount);
            }
            swapTokensForEth(
    ...
    function _takeBuyFees(address to ,address from, uint256 amount) private returns (uint256) {
        ...
        if (to == uniswapV2Pair && from != address(uniswapV2Router) && !_isExcludedFromFee[from]) {
            tAmount = amount.mul(_SellTax).div(100);
            if (tAmount > 0) {
            uint256 burnAmount = tAmount.mul(1).div(100);
            ...
            tAmount = amount.mul(_BuyTax).div(100);
            if (tAmount > 0) {
            uint256 burnAmount = tAmount.mul(5).div(100);
